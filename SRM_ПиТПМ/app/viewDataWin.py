from PyQt6.QtWidgets import (QWidget, QPushButton, QVBoxLayout,
                             ...
                             QMessageBox, QLineEdit, QHBoxLayout, QComboBox)  # отсутствовал QComboBox
...
class ViewDataWin(QWidget):
    ...
    def initUI(self):
        self.setWindowIcon(QIcon('resources/computer.ico'))  # добавлено
        ...
        h_l1.addWidget(self.back_button)  # добавление кнопки "Назад" в layout
        ...
        self.del_entry.clicked.connect(self.delite_order)  # подключено
    ...
    def load_data(self):
        ...
        if query_type == "all_orders":
            ...
            self.load_orders('surname')  # опечатка "surnam"
    ...
    def load_orders(self, column=None):
        ...
        self.table.setColumnCount(7)  # было 3
        self.table.setHorizontalHeaderLabels(
            ['ID заказа', 'Тип работы', 'Описание', 'Дата принятия', 'Клиент', 'Исполнитель', 'Статус'])
        self.db.get_all_orders(column, self.filter.text())
        if isinstance(self.db.data, list):
    ...
    def delite_order(self):
        if self.table.selectedItems():
            ...
            if user_response == QMessageBox.StandardButton.Yes:  # исправлено NO на YES
